...
namespace WindowsFormsAppQuanly
{
    internal class Modify
    {
       ...
       public List<NHANVIEN> TaiKhoans(string query)//check tai khoan
        ...
        public bool insertTrahang(trahang1 traHang)
        {
            SqlConnection sqlConnection = Connection.GetSqlConnection();
            string query = "insert into TRAHANG values (@SoPT,@Ngaytra,@HTTT,@MaNV,@MaNCC)";
            try
            {
                sqlConnection.Open();
                sqlCommand = new SqlCommand(query, sqlConnection);
                sqlCommand.Parameters.Add("@SoPT", SqlDbType.VarChar).Value = traHang.Sopt;
                sqlCommand.Parameters.Add("@Ngaytra", SqlDbType.DateTime).Value = traHang.Ngaytra;
                sqlCommand.Parameters.Add("@HTTT", SqlDbType.NVarChar).Value = traHang.Httt;
                sqlCommand.Parameters.Add("@MaNV", SqlDbType.VarChar).Value = traHang.Manv;
                sqlCommand.Parameters.Add("@MaNCC", SqlDbType.VarChar).Value = traHang.Mancc;
                sqlCommand.ExecuteNonQuery();
            }
            catch
            {
        ...
        public bool updateTrahang(trahang1 traHang)
        {
            SqlConnection sqlConnection = Connection.GetSqlConnection();
            string query = "update TRAHANG set Ngaytra = @Ngaytra, HTTT = @HTTT, MaNV = @MaNV, MaNCC = @MaNCC where SoPT = @SoPT";
            try
            {
                sqlConnection.Open();
                sqlCommand = new SqlCommand(query, sqlConnection);
        ...
        public bool deleteTrahang(string id)
        {
            SqlConnection sqlConnection = Connection.GetSqlConnection();
            string query = "DELETE FROM TRAHANG WHERE SoPT = @SoPT";
            try
            {
                sqlConnection.Open();
                sqlCommand = new SqlCommand(query, sqlConnection);
                sqlCommand.Parameters.Add("@SoPT", SqlDbType.NVarChar).Value = id;
                sqlCommand.ExecuteNonQuery();
            }
            catch
            {
